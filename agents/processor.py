...
INTENT_PATTERNS = [
    ...
    ("ask_location", re.compile(r"\bwhere\s+is\b")),
    ("ask_location", re.compile(r"\bhow\s+do\s+i\s+get\s+to\b")),
    
    # asking about time / schedule
    ("ask_time", re.compile(r"\bwhen\s+is\b")),
    ("ask_time", re.compile(r"\bwhat\s+time\s+does\b")),
    
    # entry requirements / course info
    ("ask_entry_requirements", re.compile(r"\bentry\s+requirements\b")),
    ("ask_course_info", re.compile(r"\bwhat\s+do\s+i\s+study\b|\bwhat\s+will\s+i\s+learn\b")),
]
...
class ProcessorAgent:
    ...
    def _map_intent_to_domain(self, intent: Optional[str]) -> Optional[str]:
        ...
        # super simple mapping – refine later
